...
public:
	...
	{
		m_strTitle = _L(_T("DialogTitles|Run"), _T("Run"));
	...
	BEGIN_TOOLTIP_MAP()
		TOOLTIP_HANDLER(IDC_NOWAIT, _L(_T("Help|Run|NoWait"), _T("If this flag is specified, it will not wait for the process to finish executing before proceeding to the next [Run] entry, or completing Setup. Cannot be combined with waituntilidle.")))
		TOOLTIP_HANDLER(IDC_RUN_PARAMETERS, _L(_T("Help|Run|Parameters"), _T("Optional command line parameters for the program, which can include constants.")))
		TOOLTIP_HANDLER(IDC_RUN_WORKINGDIR, _L(_T("Help|Run|WorkingDir"), _T("The directory in which the program is started from. If this parameter is not specified or is blank, it uses the directory from the Filename parameter; if Filename does not include a path, it will use a default directory. This parameter can include constants.")))
		TOOLTIP_HANDLER(IDC_SHELLEXEC, _L(_T("Help|Run|ShellExec"), _T("This flag is required if Filename is not a directly executable file (an .exe or .com file). When this flag is set, Filename can be a folder or any registered file type -- including .hlp, .doc, and so on. The file will be opened with the application associated with the file type on the user's system, the same way it would be if the user double-clicked the file in Explorer.\r\n\r\nWhen using a folder name in Filename it's recommended that you follow it by a backslash character (e.g. \"{group}\\\"), to ensure that a program of the same name is not executed.\r\n\r\nThere is one drawback to using the shellexec flag: it cannot wait until the process terminates. Therefore, it always works as if the nowait flag was specified.")))
		TOOLTIP_HANDLER(IDC_RUN_FILENAME, _L(_T("Help|Run|FileName"), _T("The program to execute, or file/folder to open. If Filename is not an executable (.exe or .com) or batch file (.bat or .cmd), you must use the shellexec flag on the entry. This parameter can include constants.")))
		TOOLTIP_HANDLER(IDC_WAITUNTILIDLE, _L(_T("Help|Run|WaitUntilIdle"), _T("If this flag is specified, it will pause until the process is waiting for user input with no input pending, instead of waiting for the process to terminate. (This calls the WaitForInputIdle Win32 function.) Cannot be combined with nowait.")))
		TOOLTIP_HANDLER(IDC_RUN_RUNMINIMIZED, _L(_T("Help|Run|RunMinimized"), _T("If this flag is specified, it will launch the program or document in a minimized window.")))
		TOOLTIP_HANDLER(IDC_RUN_SKIPIFDOESNTEXIST, _L(_T("Help|Run|SkipOfDoesntExist"), _T("If this flag is specified in the [Run] section, Setup won't display an error message if Filename doesn't exist.\r\n\r\nIf this flag is specified in the [UninstallRun] section, the uninstaller won't display the \"some elements could not be removed\" warning if Filename doesn't exist.")))
		TOOLTIP_HANDLER(IDC_RUN_RUNONCEID, _L(_T("Help|Run|RunOnceID"), _T("Valid only in an [UninstallRun] section. If the same application is installed more than once, \"run\" entries will be duplicated in the uninstall log file. By assigning a string to RunOnceId, you can ensure that a particular [UninstallRun] entry will only be executed once during uninstallation. For example, if two or more \"run\" entries in the uninstall log have a RunOnceId setting of \"DelService\", only the latest entry with a RunOnceId setting of \"DelService\" will be executed; the rest will be ignored. Note that RunOnceId comparisons are case-sensitive.")))
		TOOLTIP_HANDLER(IDC_RUN_POSTINSTALL, _L(_T("Help|Run|PostInstall"), _T("Valid only in an [Run] section. Instructs Setup to create a checkbox on the Setup Completed wizard page. The user can uncheck or check this checkbox and thereby choose whether this entry should be processed or not. Previously this flag was called showcheckbox.")))
		TOOLTIP_HANDLER(IDC_RUN_UNCHECKED, _L(_T("Help|Run|UnChecked"), _T("Valid only in an [Run] section. Instructs Setup to initially uncheck the checkbox. The user can still check the checkbox if he/she wishes to process the entry. This flag is ignored if the postinstall flag isn't also specified.")))
		TOOLTIP_HANDLER(IDC_RUN_DESCRIPTION, _L(_T("Help|Run|Description"), _T("Valid only in a [Run] section. The description of the entry, which can include constants. This description is used for entries with the postinstall flag. If the description is not specified for an entry, Setup will use a default description. This description depends on the type of the entry (normal or shellexec).")))
		TOOLTIP_HANDLER(IDC_RUN_SKIPIFNOTSILENT, _L(_T("Help|Run|SkipIfNotSilent"), _T("Valid only in an [Run] section. Instructs Setup to skip this entry if Setup is not running (very) silent.")))
		TOOLTIP_HANDLER(IDC_RUN_SKIPIFSILENT, _L(_T("Help|Run|SkipIfSilent"), _T("Valid only in an [Run] section. Instructs Setup to skip this entry if Setup is running (very) silent.")))
		TOOLTIP_HANDLER(IDC_RUN_RUNMAXIMIZED, _L(_T("Help|Run|RunMaximized"), _T("If this flag is specified, it will launch the program or document in a maximized window.")))
		TOOLTIP_HANDLER(IDC_RUN_STATUSMSG, _L(_T("Help|Run|StatusMsg"), _T("Valid only in a [Run] section. Determines the message displayed on the wizard while the program is executed. If this parameter is not specified or is blank, a default message of \"Finishing installation...\" will be used. This parameter can include constants.")))
		TOOLTIP_HANDLER(IDC_RUN_HIDEWIZARD, _L(_T("Help|Run|HideWizard"), _T("If this flag is specified, the wizard will be hidden while the program is running.")))
		TOOLTIP_HANDLER(IDC_RUNHIDDEN, _L(_T("Help|Run|RunHidden"), _T("If this flag is specified, it will launch the program in a hidden window. Never use this flag when executing a program that may prompt for user input.")))
		TOOLTIP_HANDLER(IDC_RUN_32BIT, _L(_T("Help|Run|32Bit"), _T("Causes the {sys} constant to map to the 32-bit System directory when used in the Filename and WorkingDir parameters. This is the default behavior in a 32-bit mode install.")))
		TOOLTIP_HANDLER(IDC_RUN_64BIT, _L(_T("Help|Run|64Bit"), _T("Causes the {sys} constant to map to the 64-bit System directory when used in the Filename and WorkingDir parameters. This is the default behavior in a 64-bit mode install.")))
		TOOLTIP_HANDLER(IDC_RUN_VERB, _L(_T("Help|Run|Verb"), _T("Specifies the action to be performed on the file. Must be combined with the shellexec flag. Commonly available verbs include \"open\" and \"print\". If this parameter is not specified or is blank, the default verb for the file type will be used (typically \"open\").")))
		TOOLTIP_HANDLER(IDC_RUNASCURRENTUSER, _L(_T("Help|Run|RunAsCurrentUser"), _T("If this flag is specified, the spawned process will inherit Setup/Uninstall's user credentials (typically, full administrative privileges).\n\nThis is the default behavior when the postinstall flag is not used.\n\nThis flag cannot be combined with the runasoriginaluser flag.")))
		TOOLTIP_HANDLER(IDC_RUNASORIGINALUSER, _L(_T("Help|Run|RunAsOriginalUser"), _T("Valid only in a [Run] section. If this flag is specified and the system is running Windows Vista or later, the spawned process will execute with the (normally non-elevated) credentials of the user that started Setup initially (i.e., the \"pre-UAC dialog\" credentials).\n\nThis is the default behavior when the postinstall flag is used.\n\nIf a user launches Setup by right-clicking its EXE file and selecting \"Run as administrator\", then this flag, unfortunately, will have no effect, because Setup has no opportunity to run any code with the original user credentials. The same is true if Setup is launched from an already-elevated process. Note, however, that this is not an Inno Setup-specific limitation; Windows Installer-based installers cannot return to the original user credentials either in such cases.\n\nThis flag cannot be combined with the runascurrentuser flag.")))
	END_TOOLTIP_MAP()
};
