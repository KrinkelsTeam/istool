...
public:
	...
	BEGIN_TOOLTIP_MAP()
		TOOLTIP_HANDLER(IDC_BTN_LICENSEFILE, _L(_T("Help|Application|LicenseFileButton"), _T("Click to select license file.")))
		TOOLTIP_HANDLER(IDC_BTN_INFOBEFOREFILE, _L(_T("Help|Application|InfoBeforeFileButton"), _T("Click to select info before file.")))
		TOOLTIP_HANDLER(IDC_BTN_INFOAFTERFILE, _L(_T("Help|Application|InfoAfterFileButton"), _T("Click to select info after file.")))
		TOOLTIP_HANDLER(IDC_APPCOPYRIGHT, _L(_T("Help|Application|AppCopyright"), _T("This is optional, and is only used to display a copyright message in the bottom-right corner of Setup's background window.")))
		TOOLTIP_HANDLER(IDC_APPNAME, _L(_T("Help|Application|AppName"), _T("This required directive specifies the title of the application you are installing. Do not include the version number, as the AppVerName directive is for that purpose.")))
		TOOLTIP_HANDLER(IDC_APPVERNAME, _L(_T("Help|Application|AppVerName"), _T("The value of this required directive should be the same (or similar to) the value of AppName, but it should also include the program's version number. The value may include constants.")))
		TOOLTIP_HANDLER(IDC_INFOAFTERFILE, _L(_T("Help|Application|InfoAfterFile"), _T("Specifies the name of an optional \"readme\" file, in .txt or .rtf (rich text) format, which is displayed after a successful install.")))
		TOOLTIP_HANDLER(IDC_INFOBEFOREFILE, _L(_T("Help|Application|InfoBeforeFile"), _T("Specifies the name of an optional \"readme\" file, in .txt or .rtf (rich text) format, which is displayed before the user selects the destination directory for the program.")))
		TOOLTIP_HANDLER(IDC_LICENSEFILE, _L(_T("Help|Application|LicenseFile"), _T("Specifies the name of an optional license agreement file, in .txt or .rtf (rich text) format, which is displayed before the user selects the destination directory for the program.")))
		TOOLTIP_HANDLER(IDC_CHANGESASSOCIATIONS, _L(_T("Help|Application|ChangesAssociations"), _T("When set to yes, Setup will tell Explorer to refresh its file associations information at the end of the installation, and Uninstall will do the same at the end of uninstallation.")))
		TOOLTIP_HANDLER(IDC_PASSWORD, _L(_T("Help|Application|Password"), _T("Specifies a password you want to prompt the user for at the beginning of the installation.")))
		TOOLTIP_HANDLER(IDC_APPMUTEX, _L(_T("Help|Application|AppMutex"), _T("This directive is used to prevent the user from installing new versions of an application while the application is still running, and to prevent the user from uninstalling a running application.")))
		TOOLTIP_HANDLER(IDC_APP_MINIMUM_VERSION_9X, _L(_T("Help|Application|MinVerWin"), _T("This directive lets you specify a minimum version of Windows that your software runs on.")))
		TOOLTIP_HANDLER(IDC_APP_MINIMUM_VERSION_NT, _L(_T("Help|Application|MinVerNT"), _T("This directive lets you specify a minimum version of Windows NT that your software runs on.")))
		TOOLTIP_HANDLER(IDC_APP_ONLY_BELOW_VERSION_9X, _L(_T("Help|Application|BelowVerWin"), _T("This directive lets you specify a minimum version of Windows that your software will not run on.")))
		TOOLTIP_HANDLER(IDC_APP_ONLY_BELOW_VERSION_NT, _L(_T("Help|Application|BelowVerNT"), _T("This directive lets you specify a minimum version of Windows NT that your software will not run on.")))
		TOOLTIP_HANDLER(IDC_PRIVILEGESREQUIRED, _L(_T("Help|Application|PrivilegesRequired"), _T("This directive specifies the minimum user privileges required to run the installation.")))
		TOOLTIP_HANDLER(IDC_EXTRADISKSPACEREQUIRED, _L(_T("Help|Application|ExtraDiskSpaceRequired"), _T("Normally, the disk space requirement displayed on the wizard is calculated by adding up the size of all the files in the [Files] section. If you want to increase the disk space display for whatever reason, set ExtraDiskSpaceRequired to the amount of bytes you wish to add to this figure.")))
		TOOLTIP_HANDLER(IDC_ALWAYSRESTART, _L(_T("Help|Application|AlwaysRestart"), _T("When set to yes, Setup will always prompt the user to restart the system at the end of a successful installation, regardless of whether this is necessary.")))
		TOOLTIP_HANDLER(IDC_RESTARTIFNEEDEDBYRUN, _L(_T("Help|Application|RestartIfNeededByRun"), _T("When set to yes, and a program executed in the [Run] section queues files to be replaced on the next reboot, Setup will detect this and prompt the user to restart the computer at the end of installation.")))
		TOOLTIP_HANDLER(IDC_ALLOWCANCELDURINGINSTALL, _L(_T("Help|Application|AllowCAncelDuringInstall"), _T("Setting this to no prevents the user from cancelling during the actual installation process, by disabling the Cancel button and ignoring clicks on the close button.")))
		TOOLTIP_HANDLER(IDC_TIMESTAMPSINUTC, _L(_T("Help|Application|TimeStampsInUTC"), _T("If TimeStampsInUTC is set to yes, time stamps will be saved and restored in UTC -- the native time format of Win32 and NTFS.")))
		TOOLTIP_HANDLER(IDC_TIMESTAMPROUNDING, _L(_T("Help|Application|TimeStampRounding"), _T("By default, time stamps on files referenced by [Files] section entries are rounded down to the nearest 2-second boundary. FAT partitions have only a 2-second time stamp resolution, so this ensures that time stamps are set the same way on both FAT and NTFS partitions.\r\n\r\nThe rounding can be altered or disabled by setting the TimeStampRounding directive. Setting it to 0 will disable the rounding. Setting it to a number between 1 and 60 will cause time stamps to be rounded down to the nearest TimeStampRounding-second boundary.")))
		TOOLTIP_HANDLER(IDC_TOUCHDATE, _L(_T("Help|Application|TouchDate"), _T("The date used in the time/date stamp of files referenced by [Files] section entries that include the touch flag.")))
		TOOLTIP_HANDLER(IDC_TOUCHTIME, _L(_T("Help|Application|TouchTime"), _T("The time used in the time/date stamp of files referenced by [Files] section entries that include the touch flag.")))
		TOOLTIP_HANDLER(IDC_BTN_BELOWVERSIONNT, _L(_T("Help|Application|BelowVerNTButton"), _T("Click to select version.")))
		TOOLTIP_HANDLER(IDC_BTN_BELOWVERSIONWIN, _L(_T("Help|Application|BelowVerWinButton"), _T("Click to select version.")))
		TOOLTIP_HANDLER(IDC_BTN_MINVERSIONNT, _L(_T("Help|Application|MinVerNTButton"), _T("Click to select version.")))
		TOOLTIP_HANDLER(IDC_BTN_MINVERSIONWIN, _L(_T("Help|Application|MinVerWinButton"), _T("Click to select version.")))
		TOOLTIP_HANDLER(IDC_CHANGESENVIRONMENT, _L(_T("Help|Application|ChangesEnvironment"), _T("When set to yes, at the end of the installation Setup will notify other running applications (notably Windows Explorer) that they should reload their environment variables from the registry.")))
	END_TOOLTIP_MAP()
