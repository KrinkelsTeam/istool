...
const CInnoScriptEx::CConstantInfo CInnoScriptEx::m_constants[] = {
	...
	_T("{sys}"),			_T("The system's System32 directory"), true,
	_T("{sysnative}"),		_T("64-bit system files on 64-bit Windows, 32-bit on 32-bit"), true,
	_T("{syswow64}"),		_T("32-bit system files on 64-bit Windows; same as {sys} on 32-bit"), true,
	_T("{src}"),			_T("The directory in which the Setup files are located"), true,
	_T("{sd}"),				_T("System Drive"), true,
	_T("{commonpf}"),		_T("Program Files path; maps to 32-bit or 64-bit based on install mode"), true,
	_T("{commonpf32}"),		_T("32-bit Program Files"), true,
	_T("{commonpf64}"),		_T("64-bit Windows only: 64-bit Program Files"), true,
	_T("{commoncf}"),		_T("Common Files directory; 32-bit or 64-bit based on install mode"), true,
	_T("{commoncf32}"),		_T("32-bit Common Files"), true,
	_T("{commoncf64}"),		_T("64-bit Windows only: 64-bit Common Files"), true,
	_T("{tmp}"),			_T("Temporary directory used by Setup or Uninstall"), true,
	_T("{commonfonts}"),	_T("Fonts directory"), true,
	_T("{dao}"),			_T("DAO directory"), true,
	
	_T("{dotnet11}"),		_T("32-bit .NET Framework version 1.1 install root directory."), true,
	_T("{dotnet20}"),		_T(".NET Framework version 2.0-3.5 install root directory"), true,
	_T("{dotnet2032}"),		_T("32-bit .NET Framework version 2.0-3.5 install root directory"), true,
	_T("{dotnet2064}"),		_T("64-bit Windows only: 64-bit .NET Framework version 2.0-3.5 install root directory"), true,
	_T("{dotnet40}"),		_T(".NET Framework version 4.0 and later install root directory"), true,
	_T("{dotnet4032}"),		_T("32-bit .NET Framework version 4.0 and later install root directory"), true,
	_T("{dotnet4064}"),		_T("64-bit Windows only: 64-bit .NET Framework version 4.0 and later install root directory"), true,

	// Shell Folder Constants 
	_T("{group}"),				_T("User-selected Start Menu folder path"), true,
	_T("{localappdata}"),		_T("The path to the current user's local Application Data folder"), true,
	...
	_T("{usercf}"),				_T("The path to the current user's Common Files folder (Win 7+)"), true,
	...
	_T("{userfavorites}"),		_T("The path to the current user's Favorites folder"), true,
	_T("{userfonts}"),			_T("The path to the current user's Fonts folder (Win 10 1803+)"), true,
	_T("{userpf}"),				_T("The path to the current user's Program Files folder (Win 7+)"), true,
	...
	_T("{usersavedgames}"),		_T("The path to the current user's Saved Games folder"), true,
	_T("{usersendto}"),			_T("The path to the current user's Send To folder"), true,
	...
	// Auto Constants
	_T("{autoappdata}"),		_T("{commonappdata} or {userappdata} based on privileges"), true,
	_T("{autocf}"),				_T("{commoncf} or {usercf} based on privileges"), true,
	_T("{autocf32}"),			_T("{commoncf32} or {usercf} based on privileges"), true,
	_T("{autocf64}"),			_T("{commoncf64} or {usercf} based on privileges"), true,
	_T("{autodesktop}"),		_T("{commondesktop} or {userdesktop} based on privileges"), true,
	_T("{autodocs}"),			_T("{commondocs} or {userdocs} based on privileges"), true,
	_T("{autofonts}"),			_T("{commonfonts} or {userfonts} based on privileges"), true,
	_T("{autopf}"),				_T("{commonpf} or {userpf} based on privileges"), true,
	_T("{autopf32}"),			_T("{commonpf32} or {userpf} based on privileges"), true,
	_T("{autopf64}"),			_T("{commonpf64} or {userpf} based on privileges"), true,
	_T("{autoprograms}"),		_T("{commonprograms} or {userprograms} based on privileges"), true,
	_T("{autostartmenu}"),		_T("{commonstartmenu} or {userstartmenu} based on privileges"), true,
	_T("{autostartup}"),		_T("{commonstartup} or {userstartup} based on privileges"), true,
	_T("{autotemplates}"),		_T("{commontemplates} or {usertemplates} based on privileges"), true,


	// Other Constants 
	_T("{\\}"),												_T("A backslash character"), false,
	_T("{%NAME,DefaultValue}"),								_T("Embeds an environment variable"), false,
	_T("{cmd}"),											_T("The full pathname of the system's standard command interpreter"), false,
	_T("{computername}"),									_T("The name of the computer the Setup or Uninstall program is running on"), false,
	_T("{drive:Path}"),										_T("Extracts the drive letter and colon from path"), false,
	_T("{groupname}"),										_T("Group selected on Select Program Group wizard page"), false,
	_T("{wizardhwnd}"),										_T("Translates to the window handle of the Setup wizard window"), false,
	_T("{ini:Filename,Section,Key|DefaultValue}"),			_T("Embeds a value from an .INI file"), false,
	_T("{language}"),										_T("The internal name of the selected language"), false,
	_T("{cm:MessageName}"),									_T("Embeds a custom message value based on the active language"), false,
	_T("{cm:MessageName,Arguments}"),						_T("Embeds a custom message value based on the active language"), false,
	...
	_T("{param:ParamName|DefaultValue}"),					_T("Embeds a command line parameter value"), false,
	_T("{srcexe}"),											_T("The full pathname of the Setup program file"), false,
	_T("{uninstallexe}"),									_T("The full pathname of the uninstall program extracted by Setup"), false,
	_T("{sysuserinfoname}"),								_T("The name that Windows is registered to"), false,
	...
	_T("{userinfoserial}"),									_T("Serial number entered by the user"), false,
	_T("{username}"),										_T("The name of user who is running Setup program"), false,
