...
void CMyListView::WriteRegistryInfo(WORD* pDisplayInfo, UINT nCount) {
	...
	strClassName.Format(_T("LVx%02X"), GetSection());
	LPCTSTR lpszName = strClassName;

	CString str;
	str.Format(_T("%sColumns"), lpszName);
	AfxGetApp()->WriteProfileBinary(_T("Settings"), str, (LPBYTE)pDisplayInfo, nCount * sizeof WORD);
...
bool CMyListView::ReadRegistryInfo(WORD* pDisplayInfo, UINT nCount) {
	...
	if (!AfxGetApp()->GetProfileBinary(_T("Settings"), str, pInfo, &nCount2))
		return false;
...
LRESULT CMyListView::OnNewItem(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
	...
	if (GetItemCount() == 1 && GetSection() == CInnoScript::PRJ_DOWNLOAD) {
		GetDocument()->GetScript().AddLine(new CInnoScript::CLineComment(CInnoScript::SEC_CODE, _T("// Function generated by ISTool.")));
		GetDocument()->GetScript().AddLine(new CInnoScript::CLineComment(CInnoScript::SEC_CODE, _T("function NextButtonClick(CurPage: Integer): Boolean;")));
		GetDocument()->GetScript().AddLine(new CInnoScript::CLineComment(CInnoScript::SEC_CODE, _T("begin")));
		GetDocument()->GetScript().AddLine(new CInnoScript::CLineComment(CInnoScript::SEC_CODE, _T("	Result := istool_download(CurPage);")));
		GetDocument()->GetScript().AddLine(new CInnoScript::CLineComment(CInnoScript::SEC_CODE, _T("end;")));
	}
